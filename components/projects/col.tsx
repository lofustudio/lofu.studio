...
export default function ProjectCol({ href, title, desc, year, img }: { href: string, title: string, desc: string, year: number, img: string }) {
  ...
  return (
    <>
      ...
      <a className="flex flex-row items-center w-full p-2 overflow-hidden rounded-md whitespace-nowrap text-ellipsis hover:bg-neutral-100 dark:hover:bg-neutral-900" target="_blank" href={href} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
